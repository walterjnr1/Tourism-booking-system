...
if (isset($_POST['btnsave'])) {
    ...
    if ($image['error'] == 0) {
        ...
        if (in_array($imageExtension, array('jpg', 'jpeg', 'png'))) {
            // Resize the image
            $tmpImage = imagecreatefromstring(file_get_contents($image['tmp_name']));
            ...
            $tmpFilePath = 'tmp_' . $imageName;
            if ($imageExtension == 'jpg' || $imageExtension == 'jpeg') {
                imagejpeg($newImage, $tmpFilePath);
            } elseif ($imageExtension == 'png') {
                imagepng($newImage, $tmpFilePath);
            }

            // Upload the resized image to the server
            $uploadedImage = "uploadImage/places/" . $imageName;
            move_uploaded_file($tmpFilePath, "../" . $uploadedImage);

            // Insert the data into the database
            $query = "INSERT INTO places (name, address, category, amount, description, photo, capacity) VALUES ('$name', '$address', '$category', '$amount', '$description', '$uploadedImage', '$capacity')";
...
<select class="form-control" name="cmdcategory" id="exampleInputCategory">
    ...
    ?>
                  ...
                  <div class="form-group">
                    <label for="exampleInputPassword1">No. of people per Time</label>
                    <input type="number" class="form-control" name="txtcapacity" id="txtcapacity" size="77" value="<?php if (isset($_POST['txtcapacity']))?><?php echo $_POST['txtcapacity']; ?>" >
                  </div>
                  <div class="form-group">
