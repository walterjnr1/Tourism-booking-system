...
<script type="text/javascript">
		function Finish(){
if(confirm("ARE YOU SURE YOU WISH TO FINISH THIS TOUR ?" ))
{
return  true;
}
else {return false;
...
$sql = "SELECT tblusers.name,tblbooking.*,places.name as place_name FROM tblusers INNER JOIN tblbooking ON tblusers.username = tblbooking.user INNER JOIN places ON tblbooking.place = places.id order by tblbooking.id asc";
         ...
         <?php if (($row1['status'])==(('Approved')))  { ?>
              <a button class="btn btn-success"  href="approve_finish_tour.php?id=<?php echo $row['id'];?>" onClick="return Finish();">Finish</button></a>
					  <?php } else {?>
              <a button class="btn btn-success"  href="approve_finish_tour.php?uid=<?php echo $row['id'];?>" onClick="return Approved();">Approved</button></a>
					  <?php } ?>
        </td>
                    </tr>
					<?php $cnt=$cnt+1;} ?>
