...
$stmt = $dbh->query("SELECT * FROM tblbooking where place='$id' and status='Ongoing'");
$count_booking = $stmt->rowCount();

//check if capacity is full
if ($count_booking >= $capacity) {
    $_SESSION['error'] = 'Sorry, this place is full';
...
$query = "INSERT INTO tblbooking (user,booking_id, place, check_in, check_out,amount,status) VALUES ('$user','$bookingid', '$id', '$start', '$end','$amount','Ongoing')";
$result_booking = mysqli_query($conn, $query);
if ($result_booking) {
$_SESSION['success'] = 'Booking was successful!';
 
} else {
$_SESSION['error'] = 'Failed to book successfully';
}
}
}
?>
...
<head>
    <meta charset="utf-8">
    <title>Welcome - <?php echo $sit ?></title>
...
if ($result->num_rows > 0) {
  ...
  while($row = $result->fetch_assoc()) {
   ...
   <div class="col-lg-4 col-md-6 mb-4" id="form">
