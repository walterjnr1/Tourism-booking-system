...
// Get the capacity from the places table
$capacityQuery = "SELECT capacity FROM places WHERE id = ?"; // Replace ? with the actual place ID

// Prepare the query
$stmt = $conn->prepare($capacityQuery);

// Bind the place ID parameter
...
// Get the current bookings for the place
$bookingsQuery = "SELECT COUNT(*) as bookings FROM tblbooking WHERE place_id = ?"; // Replace ? with the actual place ID
...
$currentBookings = $stmt->get_result()->fetch_assoc()['bookings'];

// Check if the capacity has been exceeded
if ($currentBookings >= $capacity) {
    // Capacity exceeded, display an error message
    echo "Sorry, the capacity for this place has been exceeded.";
} else {
    // Capacity not exceeded, proceed with the booking
    // Insert the booking into the tblbooking table
    $bookingQuery = "INSERT INTO tblbooking (place_id, ...) VALUES (?, ...)"; // Replace ? with the actual values
...
<div class="input-group mb-3">
  ...
  <input type="date" name="txtend" id="txtend" class="form-control txtend" placeholder="Choose End Date" required>
     ...
     <input type="hidden" id="hiddays" name="hiddays" >
...
<div class="border-top mt-4 pt-1">
  ...
  <h5 class="m-0 style1" id="total">Total- N0</h5>
    ...
    <p class="m-0 days"></p>
                            ...
                            <div>
                                    <button name="" class="btn btn-primary btn-block py-3" type="submit">Book</button>
                            </div>
                        </div>
                    </div>
                </div>
